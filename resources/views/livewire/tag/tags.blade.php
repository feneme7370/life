                ...
                <tbody>
                    @forelse ($tags as $tag)
                        <tr class="hover:bg-gray-50 hover:text-gray-800 border-b border-gray-300 text-center">
                            <td class="px-3 py-1 whitespace-nowrap">{{ $tag->name }}</td>
                            <td class="px-3 py-1 whitespace-nowrap">
                                <div class="flex gap-1 justify-center items-center">
                                    <a wire:click="edit('{{ $tag->uuid }}')">
                                        <flux:button variant="ghost" color="blue" icon="pencil-square" size="sm"></flux:button>
                                    </a>
                                    
                                    <flux:button variant="ghost" color="red" icon="trash" size="sm" wire:click="delete('{{ $tag->uuid }}')"></flux:button>
                                </div>
                            </td>
                        </tr>
                    @empty
